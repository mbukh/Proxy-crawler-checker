def detect_proxies_type(queue_proxies: list = [], save_anonymous: bool = True, debug: bool = False) -> set:
    ...
    def checkProxy(proxy: str = '') -> str:
        ...
        # CHECK SERVER APIs
        checkUrls = ["https://api.ipify.org/", # txt or https://api.ipify.org?format=json
        ...
        for checkerUrl in randomCheckers:
            ...
            for protocol in protocols:
                try:
                    ...
                    if debug:
                        print("[ERROR]:", "Couldn't connect to", checkerUrl, "via proxy", protocol)
                    pass
        if debug:
    ...
    print("Working proxies", len(export_proxies), "/", len(queue_proxies), ", Anonymous:", len(anonym_proxies) if my_ip else "couldn't check")
...
if __name__ == "__main__":
    ...
    print(
        detect_proxies_type(queue_proxies=queue_proxies, save_anonymous = False, debug = False)
    )
