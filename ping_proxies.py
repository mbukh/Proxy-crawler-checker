def ping_proxies(hosts_list: list = []) -> set:
    ...
    def ping(host):
        response = subprocess.call(["ping", "-c 2", "-t "+str(TMOUT), host.split(":")[0]], stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        if response == 0:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=35) as executor: # no "max_workers" for optimally defined number of threads
